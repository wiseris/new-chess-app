...
@NoArgsConstructor
@ToString
public class QueenData extends FigureData {
    ...
    public ActionResult validateAction(GameData gameData, ActionCommand action) {
        ...
        if (getPosition().equals(targetPosition) ||
                (
                        !targetPosition.isSameColumn(getPosition()) &&
                        !targetPosition.isSameRow(getPosition()) &&
                        !targetPosition.isSameDiagonal(getPosition())
                )
        ) {
        ...
        for(FigureData figure: gameData.getBoard().values()) {
            ...
            if (    figure.getPosition().isDiagonalBetween(getPosition(), targetPosition)
    ...
    public boolean canAttack(GameData gameData, CellPosition targetPosition, FigureData exclude, CellPosition replacedPosition) {
        ...
        for(FigureData figure: gameData.getBoard().values()) {
            ...
            if (
                    figurePosition.isDiagonalBetween(getPosition(), targetPosition)
